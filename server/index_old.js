...
const ports = (await WindowsBinding.list()).filter((p)=>(p.path=='/dev/ttyUSB0'||p.path=='COM2'));
...
const readInitBytes = async () => {
	...
	console.log('ECU address: ', ecuAddress);
}
...
const sendCommandISO9141 = async (command, responseLen, requestName) => {
	let commandBuffer = Buffer.from(command, 'hex');
...
await wakeupECU();
// const res = await sendCommandISO9141('686AF1010C', 7)
// console.log(res)
